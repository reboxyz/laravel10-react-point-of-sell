    ...
    public function store(Request $request): JsonResponse
    {
        try {
            $validator = Validator::make($request->all(), [
                ...
                'zip_code' => 'required|string|max:8|min:6',
            ]);
    ...
    public function show(string $id): JsonResponse
    {
        try {
            ...
            if (empty($data["customer"])) {
                return $this->sendError("Customer not found", ["errors" => [
                    "generic" => "Customer not found"
                ]], 404);
            }
        ...
        try {
            ...
            if (!empty($search)) {
                $query->where(function($query) use ($search) {
                    $query->orWhere(DB::raw("first_name || ' ' || last_name"), 'like', '%'.$search .'%' );
            ...
            $data['customers'] = $query->orderBy('first_name')->limit(100)->get([
                'id',
                DB::raw("first_name || ' ' || last_name as label")
            ]);
